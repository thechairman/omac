public class getFixData{
		...
		for(int files = 0; files < args.length; files++){
			...
			while(cin.hasNextLine()){
               ...
               // System.out.println("String is: " + str);
				...
				String stateString = tokens[1].trim(); 
				System.out.print(stateString+"\t");

				str = str.substring(str.indexOf(' ')+1);
				str = str.trim();
				try{       
					currentTime = java.lang.Double.parseDouble(tokens[0].trim());
	...
	//			java.lang.String stateString = str.substring(str.indexOf(' '));
				...
				System.out.println(theNode.state);
				...
				switch(theNode.state){
					case TX:
						if(currentTime < theNode.lastTime){
							skip_flag = 1;
							System.out.println(currentTime);
							System.out.println(theNode.lastTime);
							continue;
						}
						if(skip_flag != 1){
